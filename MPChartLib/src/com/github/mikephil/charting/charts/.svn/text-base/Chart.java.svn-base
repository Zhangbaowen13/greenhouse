...
public abstract class Chart<T extends ChartData<? extends IDataSet<? extends Entry>>> extends
    ...
    private boolean isLeftMarkerView;
    /**
    ...
    protected void drawMarkers(Canvas canvas) {
        ...
        for (int i = 0; i < mIndicesToHighlight.length; i++) {
            ...
            if (xIndex <= deltaX && xIndex <= deltaX * mAnimator.getPhaseX()) {
                ...
                Log.i("lib", "pso[0]" + pos[0] + "pos[1]" + pos[1] + "mMarkerView.getHeight()" + mMarkerView.getHeight());
                // check bounds
                ...
                if (isLeftMarkerView)
				{
                	mMarkerView.draw(canvas,0,pos[1]);
				}else {
    ...
    public void setIsLeftMarkView(boolean state){
    	isLeftMarkerView = state;
    }
