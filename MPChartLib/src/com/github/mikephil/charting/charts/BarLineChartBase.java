...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends IBarLineScatterCandleBubbleDataSet<? extends Entry>>>
    ...
    protected float[] getMarkerPosition(Entry e, Highlight highlight) {
        ...
        float yPos = e.getVal();
        Log.i("lib", "Bar getMarkerPosition  xPos" + xPos + "  yPos  " + yPos);
        if (e instanceof CandleEntry)
		{
        	CandleEntry ce = (CandleEntry) e;
        	float hight = ce.getHigh();
            float low = ce.getLow();
            float open = ce.getOpen();
            float close = ce.getClose();
            if (open > close) {
            	yPos = low;
			}else if (open < close) {
				yPos = hight;
            ...
            Log.i("lib", "Bar getMarkerPosition  e instanceof CandleEntry yPos" + yPos);
		}
        ...
        if (this instanceof BarChart) {
            ...
            yPos *= mAnimator.getPhaseY();
            Log.i("lib", "Bar getMarkerPosition else yPos" + yPos);
        }
